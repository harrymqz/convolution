...
int main(int argc, char **argv) {
    ...
    long position = 0;
    double start, tstart = 0, tend = 0, tread = 0, tcopy = 0, tconv = 0, tstore = 0, treadk = 0, tsending = 0, treceive = 0;
    struct timeval tim;
    ...
    if (rank == 0) {
        ...
        //    if (rank == 0 && partitions < nprocs - 1) {
        //        partitions = nprocs - 1;
        //    }
        //    if (rank != 0) {
        //        partitions = 1;
        //    }
        //printf("\t\t\t RANK: %d, partitions %d\n", rank, partitions);
        ...
        while (c < partitions) {
            ...
            for (i = 1; i < nprocs; i++) {
                ...
                //printf("------>MASTER[%d] sending array_length = %d, partsize = %d, offset = %d,"
                 //       "height = %d to %d\n", rank, array_length, partsize, offset, height, i);
                MPI_Status status;
                start = MPI_Wtime(); 
            ...
            for (i = 1; i < nprocs; i++) {
                ...
                treceive += (MPI_Wtime()- start);
                //printf("------>MASTER[%d] received array_length = %d, partsize = %d, offset = %d"
                //        " from %d\n", rank, array_length, partsize, offset, 0);
                //                    receiveFromMaster(output, &chunksize, &partsize, &offset, i);
    ...
    if (rank != 0) {
        ...
        MPI_Status status;
        while (1) {
            //printf("--------------> Rank[%d] waiting for rank[%d] data...\n", rank, 0);
            ...
            //printf("-------------->SLAVE[%d] received array_length = %d, partsize = %d, offset = %d, height =%d, sourcer->ancho = %d"
             //       " from %d\n", rank, array_length, partsize, offset, height, source->ancho, 0);
            output = (ImagenData) malloc(sizeof (struct imagenppm));
            ...
            //printf("Sending to Master from RANK[%d]\n", rank);
            free(source->R);
            free(source->G);
            free(source->B);
            free(source);
