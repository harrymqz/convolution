...
//            if (source->R[i]!=output->R[i] || source->G[i]!=output->G[i] || source->B[i]!=output->B[i]) printf("At position i=%d %d!=%d,%d!=%d,%d!=%d\n",i,source->R[i],output->R[i], source->G[i],output->G[i],source->B[i],output->B[i]);
        ...
        #pragma omp parallel num_threads(3)
        {
            #pragma omp sections
            {
                ...
                #pragma omp section
                {
                    convolve2D(source->G, output->R, source->ancho, (source->altura/partitions)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
                }
                #pragma omp section
                {
                    convolve2D(source->B, output->R, source->ancho, (source->altura/partitions)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
                }
            }
        }
        gettimeofday(&tim, NULL);
